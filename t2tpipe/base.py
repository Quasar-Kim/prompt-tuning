...
from t2tpipe.dataclass import ModelTrainOutput, ModelPredictionOutput, Env, BatchTextPrediction
...
class BaseLightningModule(LightningModule, ABC):
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> BatchTextPrediction:
    ...
    def _collate_prediction_outputs(self, outputs: List[ModelPredictionOutput]) -> ModelPredictionOutput:
        ys = [] # list of (B, N)
        y_preds = [] # list of (B, N)
        for out in outputs:
            ys.append(out.x)
    ...
    def _collate_train_outputs(self, outputs: List[ModelTrainOutput]) -> ModelTrainOutput:
        ys = [] # list of (B, N)
        y_preds = [] # list of (B, N)
        losses = [] # list of (B,)
        for out in outputs:
...
def _model_prediction_output_to_cpu(outputs: ModelPredictionOutput) -> ModelPredictionOutput:
