...
class TorchDataWrappingTransformDataPipe(TransformDataPipe):
    ...
    def source_dp(self, dp: DataPipe):
        ...
        self._torch_datapipe = self._torch_datapipe_cls(
            self._source_dp, *self._args, **self._kwargs
        )  # type: ignore
...
class DistributedShardingFilter(TransformDataPipe):
    ...
    def __len__(self):
        assert hasattr(
            self.source_dp, "__len__"
        ), "DistributedShardingFilter requires source_dp to have length, but it does not"
...
class Padder(Mapper):
    ...
    def _pad_train_sample(self, sample: EncodedSampleForTrain):
        return EncodedSampleForTrain(
            x=self._pad_tensor(sample.x), y=self._pad_tensor(sample.y)
        )

    def _pad_prediction_sample(self, sample: EncodedSampleForPrediction):
        return EncodedSampleForPrediction(x=self._pad_tensor(sample.x))
...
class PadderForEncDecModel(BasePadder):
    ...
    def _pad_prediction_sample(
        self, sample: EncDecSampleForPrediction
    ) -> EncDecSampleForPrediction:
        padded_enc_x, enc_attention_mask = self._pad_tensor(sample.enc_x)
