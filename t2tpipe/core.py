    ...
    if env.model.padder is None:
        ...
        if default_padder is not None:
            env = dataclasses.replace(
                env, 
                model=dataclasses.replace(env.model, padder=default_padder)
            )
    if env.task.postprocessor is None:
        noop_postprocessor = NoopPostProcessor()
        env = dataclasses.replace(
            env,
            task=dataclasses.replace(env.task, postprocessor=noop_postprocessor)
        )
    if env.task.metrics is None:
        env = dataclasses.replace(
            env,
            task=dataclasses.replace(env.task, metrics=[])
        )
    return env
